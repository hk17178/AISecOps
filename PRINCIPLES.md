...
### P-18. 配置走 Web 页面，不走命令行/改文件
- 运行期配置（LLM provider/API key、预算、出域开关、数据源连接、通知、RBAC）都在「系统设置」页管理，点填即改、热生效。
- **不让用户** `cp .env` / 编辑文件 / 重启服务。
- 实现：配置存 DB + UI 编辑；密钥**加密存储**；配置读取做成**源无关**（现读 .env，将来从 DB 读）。`.env` 仅首次引导/开发用。
- 理由：自用内部工具，"用着方便" > "工程便利"。用户硬性偏好 [[feedback-web-config]]
