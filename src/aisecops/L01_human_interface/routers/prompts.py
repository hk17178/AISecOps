...
async def rollback_prompt(key: str, body: PromptRollbackIn) -> dict[str, Any]:
    """回滚：把活跃版本指回指定旧版本。"""
    pv = rt.prompts.rollback(key, body.version, body.actor)
    if pv is None:
        raise HTTPException(status_code=404, detail=f"{key} 无版本 {body.version}")
    rt.ctx.audit.append(actor=body.actor, action="prompt_rollback", target=key, details={"version": body.version})
...
async def save_prompt(key: str, body: PromptSaveIn) -> dict[str, Any]:
    ...
    pv = rt.prompts.save(key, body.content, body.note.strip(), body.actor)
    rt.ctx.audit.append(actor=body.actor, action="prompt_save", target=key, details={"version": pv.version})
    return pv.model_dump()
