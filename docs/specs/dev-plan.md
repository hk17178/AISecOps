...
### Sprint 1：基础设施（2 周）· 🟡 进行中（2026-06-04）

> 进度：S1.2 环境 ✅ · S1.3 Docker 文件 ✅(待装 Docker 跑通) · S1.4 CI ✅ · S1.5 Makefile+OTel ✅ · S1.6 重启手册 ✅ · S1.1 决策 ADR-0005/6/7 ✅，但 5 条 PRD §9 待决策 ⏳ 需用户输入。`make check` 全绿。
...
### 8.0 功能级 DoD（一个"功能"算完成的硬标准）

> 详见 [功能验收清单 §0](feature-acceptance.md)。一个功能要算"完成"，**必须全部满足**：

- [ ] **真实后端逻辑**（非写死演示数据 / 非只读展示）
- [ ] **可操作**：该增删改查的全、该执行的动作真生效
- [ ] **关键/高风险动作有弹框**（填理由 / 二次确认），不裸点按钮
- [ ] **写动作进不可篡改审计链**（C-23：谁/何时/为什么）
- [ ] **持久化**（DB，P-18；内存只算过渡态）
- [ ] **可配置**（必要处可在 UI 配，尤其按场景分配大模型）
- [ ] **当用户从 UI 走一遍全流程能走通**
- [ ] **🚫 禁假按钮**：要么真生效，要么明确标"未实现"并禁用
- [ ] 更新状态矩阵（真/示意/未做）

### 8.1 工程级 DoD（每个 Sprint）
