...
> **进度（2026-06-04）**：**S0–S10 + S7–S9 ✅ 已实现 + 测试**（20 测试，make check + CI 全绿）。
> 含真 provider(OpenAI 兼容,API key)、出域脱敏/开关(C-32)、schema 校验(C-21)、cross-check(C-27)。
> 剩 **S11 成本看板端点** / **S12 覆盖率 ≥80% 收尾**。
