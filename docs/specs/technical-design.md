...
- 外部工具必经 L06（P-3）——治理语义不变；内部形态默认薄适配器，完整 MCP Server 可选（[ADR-0004](../adr/0004-l06-mcp-optional-thin-adapter.md)）
- HITL 默认（C-8）
- 抑制幻觉（C-24~C-27）
- 自用约束：成本控制 + 单人可维护
...
### 4.3 配置管理（Web 管理优先，见 [PRINCIPLES P-18](../../PRINCIPLES.md)）

运行期配置**尽量在「系统设置」页管理**，不让用户走命令行/改文件：

- **管什么**：LLM provider/API key、月度预算、出域开关、数据源连接（ES/SIEM/EDR）、通知渠道、RBAC。
- **怎么存**：配置存 DB + UI 编辑 + 热生效；**API key 等密钥加密存储**（不明文）。
- **`.env` 角色降级**：仅"首次引导/开发"用（如启动数据库连接、首个管理员）。运行期配置以 DB 为准。
- **代码要求**：配置读取做成**源无关**——L05 等模块现读 `.env`，将来 UI 改了从 DB 读，逻辑不变、不重启。
- **落地时点**：随 L01「系统设置」页 + L02 平台核心「配置中枢」一起做（Sprint 4+）。当前 L05 用 `.env` 是临时引导。
