...
```
┌──────────────────────────────────────────────────────────────────┐
│ L01 人机交互  告警 │ 工单 │ 报表 │ Chat │ ★协作驾驶舱(Cockpit)    │
├──────────────────────────────────────────────────────────────────┤
...
┌─ ★反馈飞轮（V2 新增；🕒 V1.0 仅做"Prompt/阈值调优"半边，DSLM 再训练 P3+）┐
│                                                              │
│  L01 工单决策 / 报表标注                                      │
│    │ (分析师"确认 / 否决 / 修正"研判)                         │
│    ▼                                                         │
...
│    │                                                         │
│    ▼                                                         │
│  L04 Prompt 调优 / 阈值调优 (V1.0) ┊ DSLM 再训练 / KG 富化 (P3+) │
│    │                                                         │
│    └──► 下一轮 L02 Agent 决策更准                             │
│                                                              │
└──────────────────────────────────────────────────────────────┘
...
> 不用"相邻层 only"（那与本文数据流自相矛盾，见 [ADR-0008](adr/0008-cross-layer-dependency-dag.md)）。改用显式允许边。

✅ **允许的依赖边**
- **横切例外**：任何层经 **L05 Gateway** 调 LLM / 经 **L06 工具适配** 调外部工具 / 依赖 **L12 横切支撑**（凭证·可观测·调度）
- **编排层例外**：**L02** 可调 L03(RAG)·L04(模型/知识)·L08(分析结论)；**L07** 可调 L02(Agent)·L08(分析)
- 数据采集自下而上：L08→L09→L10→L11

❌ **禁止**
- 数据面反向：L09/L10/L11 向上调业务/Agent 层（**飞轮反馈是事件驱动**，不算反向直连）
- 绕过 L05 调 LLM / 绕过 L06 调外部工具
- 不在上述枚举边内的新跨层边（要加 → 先写 ADR）

```
L01→L02 │ L02→{L03,L04,L08} │ L07→{L02,L08} │ 任何层→{L05,L06,L12}
飞轮：L01 ──事件──▶ L02(Tuning)   ← 唯一合法"向上",事件驱动
```
...
### L01 · 人机交互
- 面板：告警 / 工单 / 报表 / Chat / **★协作驾驶舱（Ops Cockpit）**
- **★协作驾驶舱**（ADR-0012）：实时看每个故障/工单"指派给谁、处理进展、是否超 SLA"，
  管理层只读总览，少打扰技术。底层是工单的指派/进度/SLA 扩展 + HITL Channel。
- 其余与 V1 一致
...
- **DSLM 群**（log_classifier / payload_detector / fp_suppressor）：业界基线必备，但本项目 **🕒 P3+ 远期、V1.0 不实现**（自用数据量不够训，见 [ADR-0003](adr/0003-self-use-positioning.md)）
